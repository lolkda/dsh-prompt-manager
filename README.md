...
- **在哪儿切**：列表页那行的「设为当前／取消当前」，或者「压缩 · …」芯片 —— 芯片的位置与「组合」一节说的那对芯片相同（空白会话在输入框那行，有内容后搬到会话标题行最右上角），两个入口写的是同一个字段。
...
- **芯片两个位置，同一对芯片**：新会话还空着的时候（宿主那时不画会话标题栏）在**聊天页输入框下面**的工具行右侧（模型选择器左边）；发出第一条消息后搬到**会话标题行最右上角**（页面右上角那组图标左边）。位置只在「空会话 → 有内容」这一下搬一次 —— 正文区滚动、输入框长高、回合在跑、压缩在跑，都不会再推动它，所以点它不会点空。两处都是「提示词 · …」和「压缩 · …」这两个，读同一个命名空间，不会各说各的：点开是全部组合和「不用组合（按每条开关）」，选中即切换。
