...
export function parseScriptOutput(
  ...
  const text = raw.trim()
  if (text.length === 0) return { variables: {}, truncated, problems: ['脚本没有输出任何内容'] }
  let parsed: unknown
  try {
  ...
  if (keys.length === 0) return { variables: {}, truncated, problems: ['对象里没有任何键'] }
...
export class PromptScripts {
  ...
  async refresh(names?: readonly string[]): Promise<ScriptRunReport[]> {
    const wanted = names === undefined ? this.names() : names.filter((name) => this.names().includes(name))
  ...
  async run(name: string): Promise<ScriptRunReport> {
    const stored = this.read(name)
    if (stored === undefined) throw new ScriptError('unknown-script', `没有这个脚本：${name}`)
  ...
  async runSource(name: string, source: string): Promise<ScriptRunReport> {
    if (!isEntryId(name)) throw new ScriptError('invalid-name', `${JSON.stringify(name)} 不是一个可用的脚本名`)
  ...
  async save(
    ...
    if (syntax !== undefined) throw new ScriptError('invalid-source', `脚本无法解析：${syntax}`)
    ...
    for (const variable of Object.keys(report.variables)) {
      ...
      if (owner !== undefined && owner !== name) conflicts.push(`${variable}（属于 ${owner}）`)
    }
