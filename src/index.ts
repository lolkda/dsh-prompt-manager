...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  function ownerLabel(record: { source: VariableSource; detail?: string | undefined }): string {
    if (record.source === 'script') return `脚本 ${record.detail ?? '?'}`
    if (record.source === 'config') return 'config.variables'
    if (record.source === 'probe') return '探测'
    return '环境变量'
  }
  ...
  function declareOrReport(variable: string, value: string, source: VariableSource): void {
    ...
    warn(
      ctx,
      `${variable} 已经由${ownerLabel(owner)}提供（当前值 ${JSON.stringify(owner.value)}），本次 ${ownerLabel({ source })} 提供的值被忽略`,
    )
  }
  ...
  if (scriptOverrides.problems.length > 0) throw new Error(`prompt-manager: ${scriptOverrides.problems.join('; ')}`)
  ...
  ctx.effect(() => ctx.on('system-prompt/assemble', async (assembly, _context, next) => {
    ...
    const sections = out.sections.map((section) => {
      ...
      for (const reference of guarded.escaped) {
        ...
        warn(
          ctx,
          `${section.name} 的正文引用了 ${JSON.stringify(reference)}：注册表解析不了它，已按字面量渲染（改掉这处引用，或让某个来源注册这个名字）`,
        )
      }
  ...
  function reportTruncation(document: unknown, kept: number): void {
    ...
    warn(
      ctx,
      `索引里有 ${String(raw.length)} 条条目，上限是 ${String(MAX_ENTRIES)} 条：只有前 ${String(kept)} 条会进入提示词（设置页仍然列出全部）`,
    )
  }
