    ...
    ctx.inject(['llm'], (scoped) => {
        scoped.on('llm/stream', (options, next) => {
            ...
            if (messages.length === 0) {
                report(`${instruction.id}\u0000no-message`, `压缩指令 ${instruction.id} 生效中，但这次压缩请求没有消息可替换：已沿用 DSH 自带指令`);
                return next();
            }
            ...
            for (const reference of rendered.escaped) {
                report(`${instruction.id}\u0000${reference}`, `${instruction.id} 的正文引用了 ${JSON.stringify(reference)}：注册表解析不了它，已按字面量渲染（改掉这处引用，或让某个来源注册这个名字）`);
            }
            try {
                request.messages = [
                ...
                report(`${instruction.id}\u0000frozen`, `压缩指令 ${instruction.id} 无法替换这次请求（${messageOf(error)}）：已沿用 DSH 自带指令`);
                return next();
            }
