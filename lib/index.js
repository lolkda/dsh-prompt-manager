...
export function apply(ctx, config = {}) {
    ...
    function resolveCompaction() {
        ...
        if (entry === undefined || entry.kind !== 'compaction') {
            reportCompaction(`absent:${wanted}`, `压缩指令 ${wanted} 不在索引里（也可能它不是压缩条目）：本轮压缩沿用 DSH 自带指令`);
            return undefined;
        }
    ...
    function packFor(presetId) {
        ...
        if (preset.compaction.length > 0)
            addMember(preset.compaction);
    ...
    function reportDanglingMembers() {
        ...
        if ((missing.length === 0 && misplaced.length === 0) || signature === danglingReported)
            return;
        danglingReported = signature;
        const presetId = activePreset?.id ?? '';
        if (missing.length > 0) {
            warn(ctx, `组合 ${presetId} 里有 ${String(missing.length)} 条不在索引里：${missing.join('、')}`
                + '（订阅没拉回来，或上游改了文件名）。这些条目这一轮不注入。');
        }
        if (misplaced.length > 0) {
            warn(ctx, `组合 ${presetId} 里有 ${String(misplaced.length)} 条是压缩指令：${misplaced.join('、')}`
